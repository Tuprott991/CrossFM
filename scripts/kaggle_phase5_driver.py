...
INPUT_CANDIDATES = ((Path(os.environ["CROSSFM_BUNDLE"]),) if os.environ.get("CROSSFM_BUNDLE") else (
    Path("/kaggle/input/crossfm-phase5-bundle"),
    Path("/kaggle/input/datasets/tuktuai/crossfm-phase5-bundle"),
))
INPUT = INPUT_CANDIDATES[0]
WORKING = Path(os.environ.get("CROSSFM_WORKING", "/kaggle/working"))
TOP_SUMMARY = WORKING / "crossfm_phase5_summary.json"
...
def main() -> None:
    ...
    if len(mounted) != 1:
        raise FileNotFoundError(f"Expected one Phase 5 package source, found {mounted}")
    INPUT = mounted[0]
    ...
    doctor = {
        "timestamp": datetime.now(timezone.utc).isoformat(), "gpus": gpu_info, "torch": torch.__version__,
        ...
        "execution": "two isolated workers; one immutable cached backbone response bank per episode",
        "audit": "direct analytical-prior arms separated from message-only arms", "torch_dtype": cfg["runtime"]["torch_dtype"],
    }
    atomic_json(run_dir / "doctor.json", doctor)
    processes, handles = [], []
    try:
        for rank in range(2):
            environment = os.environ.copy(); cpu_threads = str(cfg["runtime"].get("cpu_threads_per_worker", 2))
            environment.update({
                ...
                "NUMEXPR_NUM_THREADS": cpu_threads, "TOKENIZERS_PARALLELISM": "false", "CUDA_DEVICE_ORDER": "PCI_BUS_ID",
            ...
            command = [sys.executable, "-m", "crossfm.phase5_worker", "--config", str(config), "--output", str(run_dir),
    ...
    run_checked([sys.executable, "-m", "crossfm.phase5_aggregate", "--config", str(config),
    ...
    shutil.copy2(run_dir / "experiments.csv", WORKING / "crossfm_phase5_experiments.csv")
