...
def main() -> None:
    ...
    if len(mounted) != 1:
        raise FileNotFoundError(f"Expected one Phase 3 package source, found {mounted}")
    INPUT = mounted[0]
    ...
    try:
        for rank in range(2):
            ...
            command = [
                sys.executable, "-m", "crossfm.phase3_worker", "--config", str(config), "--output", str(run_dir),
    ...
    run_checked([
        sys.executable, "-m", "crossfm.phase3_aggregate", "--config", str(config),
    ...
    shutil.copy2(run_dir / "experiments.csv", WORKING / "crossfm_phase3_experiments.csv")
