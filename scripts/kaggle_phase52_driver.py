...
INPUT_CANDIDATES = ((Path(os.environ["CROSSFM_BUNDLE"]),) if os.environ.get("CROSSFM_BUNDLE") else (
    Path("/kaggle/input/crossfm-phase52-bundle"),
    Path("/kaggle/input/datasets/tuktuai/crossfm-phase52-bundle"),
))
INPUT = INPUT_CANDIDATES[0]
WORKING = Path(os.environ.get("CROSSFM_WORKING", "/kaggle/working"))
TOP_SUMMARY = WORKING / "crossfm_phase52_summary.json"
...
def main() -> None:
    ...
    if len(mounted) != 1:
        raise FileNotFoundError(f"Expected one Phase 5.2 package source, found {mounted}")
    INPUT = mounted[0]
    ...
    if len(gpu_info) != expected_count or any(
        normalize(expected_family) not in normalize(gpu["name"]) for gpu in gpu_info
    ):
    ...
    hf_hub_download(
        repo_id=cfg["models"]["specialist"]["id"],
        revision=cfg["models"]["specialist"]["revision"],
    ...
    doctor = {
        ...
        "message_contract": "immutable analytical anchor plus learned regularized view-logit residuals",
    ...
    try:
        for rank in range(2):
            ...
            threads = str(cfg["runtime"].get("cpu_threads_per_worker", 2))
            environment.update({
            ...
            command = [
                sys.executable, "-m", "crossfm.phase52_worker",
                "--config", str(config),
                "--output", str(run_dir),
                "--rank", str(rank),
                "--world-size", "2",
    ...
    if codes != [0, 0]:
        tails = {
            str(rank): (logs / f"worker_{rank}.log")
            .read_text(encoding="utf-8", errors="replace").splitlines()[-100:]
            for rank in range(2)
        }
        raise RuntimeError(f"Worker failure exit_codes={codes} tails={json.dumps(tails)}")
    run_checked([
        sys.executable, "-m", "crossfm.phase52_aggregate",
        "--config", str(config), "--output", str(run_dir), "--wheel", str(wheel),
    ])
    summary = json.loads((run_dir / "summary.json").read_text(encoding="utf-8"))
    summary.update({"kernel_status": "complete", "run_directory": str(run_dir), "doctor": doctor})
    atomic_json(TOP_SUMMARY, summary)
    shutil.copy2(run_dir / "experiments.csv", WORKING / "crossfm_phase52_experiments.csv")
