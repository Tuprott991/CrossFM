...
INPUT_CANDIDATES = ((Path(os.environ["CROSSFM_BUNDLE"]),) if os.environ.get("CROSSFM_BUNDLE") else (
    Path("/kaggle/input/crossfm-phase4-bundle"),
    Path("/kaggle/input/datasets/tuktuai/crossfm-phase4-bundle"),
))
INPUT = INPUT_CANDIDATES[0]
WORKING = Path(os.environ.get("CROSSFM_WORKING", "/kaggle/working"))
TOP_SUMMARY = WORKING / "crossfm_phase4_summary.json"
...
def main() -> None:
    ...
    if len(mounted) != 1:
        raise FileNotFoundError(f"Expected one Phase 4 package source, found {mounted}")
    INPUT = mounted[0]
    ...
    doctor = {
        ...
        "execution": "two isolated workers; one shared immutable backbone cache per episode; all methods cache-only",
    ...
    try:
        for rank in range(2):
            ...
            command = [
                sys.executable, "-m", "crossfm.phase4_worker", "--config", str(config), "--output", str(run_dir),
    ...
    if codes != [0, 0]:
        ...
        for rank in range(2):
            ...
            tails[str(rank)] = lines[-100:]
        raise RuntimeError(f"Worker failure exit_codes={codes} tails={json.dumps(tails)}")
    run_checked([
        sys.executable, "-m", "crossfm.phase4_aggregate", "--config", str(config),
    ...
    shutil.copy2(run_dir / "experiments.csv", WORKING / "crossfm_phase4_experiments.csv")
