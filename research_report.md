...
The first Phase 3 implementation uses a cached specialist response bank because the selected TabICL wrapper does not expose a stable differentiable conditioning-token interface. Each episode caches all declared feature-view predictions, frozen Qwen task/view/codebook embeddings, the continuous route posterior, and base predictions exactly once. A shared recurrent bridge scores views from the current language state, receives a soft specialist mixture, updates the latent language state through a residual evidence projector, and repeats for two rounds. One- and two-round models have identical parameterizations; only unroll depth differs.

For T4x2 efficiency, each worker is isolated to one physical GPU and two CPU threads. Training and validation caches are replicated for deterministic matched training, while every test seed is sharded by episode across both workers to avoid the previous 2:1 seed imbalance. After cache construction, both frozen backbones are deleted and the compact padded tensors remain resident on the GPU. The implementation uses factorized state/view projections rather than materializing concatenated pair tensors, shares bridge weights across rounds, suppresses progress-heavy I/O, writes atomic artifacts, and records both conceptual recurrent calls and the actual cached specialist-view calls.
